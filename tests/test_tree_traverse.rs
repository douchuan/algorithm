...
fn tree_height() {
    ...
    for (t, expect) in test_data {
        let tree: Tree<usize, usize> = TreeBuilder::build_in_level(t.as_slice());
        let r = tree.height();
        assert_eq!(
            expect, r,
            "tree = {:?}, expect = {:?}, r = {:?}",
...
fn empty_tree() {
    let vec = vec![];
    let tree: Tree<usize, usize> = TreeBuilder::build_in_level(vec.as_slice());
...
fn tree_topology1() {
    ...
    let tree: Tree<usize, usize> = TreeBuilder::build_in_level(tokens.as_slice());
    let r = unsafe { PreOrderVisitor::iterate(&tree) };
...
fn preorder_morris() {
    for (t, expect) in preorder_test_data() {
        let mut tree: Tree<usize, usize> = TreeBuilder::build_in_level(t.as_slice());
...
fn postorder_recursive() {
    let nodes = vec!["1", "#", "2", "3"];
    let tree: Tree<usize, usize> = TreeBuilder::build_in_level(nodes.as_slice());
