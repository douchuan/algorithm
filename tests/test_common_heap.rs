...
fn build_heap() {
    ...
    let expect = vec![16, 14, 10, 8, 7, 9, 3, 2, 4, 1];
    let mut tt = t.clone();
    heap::build_heap(&mut tt, |a, b| a >= b);
    assert_eq!(tt, expect, "t = {:?}, expect = {:?}", t, expect)
}
...
fn pop() {
    let t = vec![4, 1, 3, 2, 16, 9, 10, 14, 8, 7];
    let mut heap = heap::BinaryHeap::new(t, |a, b| a >= b);
